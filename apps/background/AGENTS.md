...
- `DavVolumeWorker` (`src/DavVolumeWorker.ts`) — Facade over the WebDAV volume (one per bucket, `DAV_VOLUME.getByName(normalizeVolumeKey)`; `setVolumeKey/deleteVolume` lifecycle): `fetch` resolves the inner path (`dav/DavContext`), then dispatches to `dav/methods/*` Commands with `DavRepository` + `DavLockGuard`. Files at `/` over `createDofsFs` + metadata in DO SQLite (`dav_nodes/dav_props/dav_locks` via `ensureDavSchema` + `dav-store/meta.ts` repository fns). No `blockConcurrencyWhile` nesting. Bucket deletion also clears bucket credentials in D1 (best-effort, FK cascades as backstop).
- `src/dav/` — `DavContext.ts` (pure `fsPathOf`/`hrefOf`/`resolveInnerPath`/`stripBase` + `..` traversal rejection) · `RangeParser.ts` (pure `parseRangeHeader`) · `DavRepository.ts` (dofs + SQL Repository: `statInner`/`readMeta`/`nodeInfo`/`listChildren|Recursive`/`upsertFile|CollectionNode`/`copyMeta`/`delete|renameCascade`) · `DavLockGuard.ts` (lock-precondition Policy) · `methods/ReadMethods|WriteMethods|PropMethods|CopyMoveMethods|LockMethods.ts` (one Command per RFC 4918 family; `PUT` reads `MAX_FILE_BYTES` via injected `AppConfiguration`).
